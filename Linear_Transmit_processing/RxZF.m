function [MSE, BER] = RxZF(simul,stream,ch1,SNR_dB)
...
s = stream;
G = inv(P'*H'*inv(Rn)*H*P)*P'*H'*inv(Rn);
...
%j_rx = (sqrt(Rs))'*P'*H'*inv(Rn)*H*P*sqrt(Rs);

j_rx = SNR*B*P'*H'*H*P/trace(P*P');

MSE = trace(Rs)-((trace(Rs))^2)/(trace(Rs)+trace(inv(j_rx)*Rs));
