function [MSE, BER,P] = TxZF(simul,stream,ch1,SNR_dB)
...
beta =sqrt(E_tr/(trace(inv(G*(H*H')*G')*Rs)));

s = stream;
P = beta*H'*G'*inv(G*H*H'*G');

n = sqrt(1/(SNR))*sqrt(1/(2))*(randn(M,chreal/B)+1i*randn(M,chreal/B));



shat = delta*G*(H*P*s+n);

%j_tx = G*H*H'*G'*(E_tr/trace(G*Rn*G'));

j_tx = SNR*B*G*H*H'*G'/trace(G*G');

MSE = trace(Rs)-((trace(Rs))^2)/(trace(Rs)+trace(inv(j_tx)*Rs));
